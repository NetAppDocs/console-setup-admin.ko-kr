...
콘솔에서 콘솔 에이전트를 배포하는 경우 IAM 사용자의 IAM 역할 또는 액세스 키의 ARN을 제공해야 합니다. 인증 방법에는 AWS에 콘솔을 배포할 수 있는 권한이 있어야 합니다. 필요한 권한은 링크: task-install-agent-aws-the Console.html#aws-permissions-agent[AWS용 에이전트 배포 정책]에 나열되어 있습니다.
