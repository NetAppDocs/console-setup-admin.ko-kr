...
에이전트 설정을 로드할 수 없습니다.
 a| 
NetApp Console은 에이전트 설정 페이지에 액세스하려고 할 때 이 메시지를 표시합니다.
 a| 
...
이 메시지는 상담원이 귀하의 지원 계정에 액세스할 수 없는 경우 표시됩니다.
 a| 
* 에이전트가 필요한 엔드포인트에 대한 아웃바운드 액세스 권한을 가지고 있는지 확인하십시오.link:reference-networking-saas-console.html["필수 네트워크 액세스 엔드포인트에 대해 자세히 알아보세요."]
...
다음 명령 중 하나를 사용하여 Console 에이전트를 확인하십시오. 모든 서비스의 상태가 _실행 중_이어야 합니다. 그렇지 않은 경우 NetApp 지원팀에 문의하십시오.
...
* link:task-agent-vm-config.html#connect-vcenter-host["Console 에이전트 상태 확인(vCenter 배포의 경우)"]
...
----
--
Podman(Red Hat Enterprise Linux 배포용)::
+
--
[source, cli]
----
...
== Console 에이전트가 요구 사항을 충족하는지 확인합니다.

Console 에이전트는 특정 네트워크 엔드포인트에 대한 액세스 권한이 필요하고, 여러 포트가 열려 있어야 하며, 하드웨어 요구 사항을 충족해야 합니다. Console 또는 에이전트 유지 관리 콘솔을 사용하여 Console 에이전트가 이러한 요구 사항을 충족하는지 확인할 수 있습니다.

link:reference-networking-saas-console.html["필요한 네트워크 액세스 포인트에 대해 자세히 알아보세요."] link:task-install-agent-on-prem.html#os-req-agent["호스트 요구 사항에 대해 자세히 알아보십시오."]
...
에이전트 유지 관리 콘솔을 사용하여 유효성 검사를 실행할 수도 있습니다. link:reference-agent-maint-console-config-checker.html["config-checker validate 명령어 사용 방법에 대해 자세히 알아보세요."]


IMPORTANT: 정부 클라우드 고객(제한된 모드)의 경우 NetApp Console은 제한된 모드 업그레이드 endpoint의 유효성을 검사하지 않습니다. 방화벽 규칙에서 해당 endpoint에 대한 액세스를 허용하는지 수동으로 확인해야 합니다.link:task-prepare-restricted-mode.html["제한된 모드에 필요한 네트워크 액세스 지점에 대해 자세히 알아보십시오."]


NOTE: *연결됨* 상태인 에이전트만 인증할 수 있습니다.

.콘솔에서 시작하는 단계
. *관리 > 에이전트*를 선택하세요.
. 확인하려는 콘솔 에이전트의 작업 메뉴를 선택하고 *유효성 검사*를 선택합니다.
+
image:screenshot-agent-validate.png["상담원의 작업 메뉴에 있는 유효성 검사 옵션을 보여주는 스크린샷입니다."]
...
[IMPORTANT]
====
* 새 에이전트 배포의 경우, NetApp은 다음 엔드포인트를 확인합니다: link:reference-networking-saas-console.html["여기에 나열됨"^]. 이 구성 확인은 업그레이드에 사용된 이전 엔드포인트를 사용할 경우 오류와 함께 실패합니다, link:reference-networking-saas-console-previous.html["여기에 나열됨"]. NetApp은 방화벽 규칙을 업데이트하여 현재 엔드포인트에 대한 액세스를 허용하고 이전 엔드포인트에 대한 액세스를 차단할 것을 가능한 한 빨리 권장합니다. link:reference-networking-saas-console-previous.html#update-endpoint-list["네트워킹을 업데이트하는 방법을 알아보세요"].
...
.단계
_com/opt/application/netapp/service-manager-2/config.json_ 파일에서 _skipConfigCheck_ 플래그를 설정하여 구성 확인을 비활성화합니다. 기본적으로 이 플래그는 false로 설정되고 구성 검사는 에이전트에 대한 아웃바운드 액세스를 확인합니다. 검사를 비활성화하려면 이 플래그를 true로 설정합니다. 이 단계를 완료하기 전에 JSON 구문에 익숙해지십시오.
...
콘솔 에이전트 관련 문제를 해결하지 못한 경우 NetApp 지원팀에 문의하는 것이 좋습니다. NetApp 지원팀에서는 콘솔 에이전트 ID를 요청하거나, 필요한 경우 콘솔 에이전트 로그를 보내달라고 요청할 수 있습니다.
...
문제가 발생한 경우, NetApp에서 문제 해결을 위해 AutoSupport 메시지를 NetApp Support로 보내달라고 요청할 수 있습니다.


NOTE: NetApp Console 부하 분산으로 인해 AutoSupport 메시지를 보내는 데 최대 5시간이 걸립니다.  긴급한 연락이 필요한 경우, 파일을 다운로드하여 직접 보내주시기 바랍니다.
