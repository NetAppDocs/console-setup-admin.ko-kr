...
summary: 클라우드 공급업체의 마켓플레이스에서 NetApp 데이터 서비스를 구독하여 시간당 요금이나 연간 계약을 통해 데이터 서비스 비용을 지불하세요.  NetApp 에서 라이선스를 구매한 경우 마켓플레이스 제공 서비스도 구독해야 합니다.  귀하의 라이센스 요금이 항상 먼저 청구되지만, 라이센스 용량을 초과하거나 라이센스 기간이 만료되면 시간당 요금이 청구됩니다. 
---
= 표준 모드에서 NetApp Console 데이터 서비스 구독
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.시작하기 전에
데이터 서비스를 구독하려면 Console 에이전트를 이미 배포해야 합니다. Console 에이전트에 연결된 클라우드 자격 증명에 마켓플레이스 구독을 연결해야 합니다.
