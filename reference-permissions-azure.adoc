...
---
= NetApp Console 에이전트에 대한 Azure 권한
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
다음 섹션에서는 NetApp Backup and Recovery 에서 권한이 사용되는 방식을 설명합니다.
...
.3+| 유연한 오케스트레이션 모드에서 Virtual Machine Scale Sets를 사용하여 Cloud Volumes ONTAP의 특정 영역을 지정합니다 | Microsoft.Compute/virtualMachineScaleSets/쓰기 | 예 | 아니요 | 아니요 
