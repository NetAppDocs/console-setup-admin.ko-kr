...
+
이 단계에서는 콘솔을 표준 모드에서 사용하는 방법을 설명하므로 제한된 모드는 비활성화 상태로 유지해야 합니다. 보안 환경이 조성되어 있고 이 계정을 콘솔 백엔드 서비스에서 연결 해제하려는 경우에만 제한된 모드를 활성화해야 합니다. 그 경우에는 link:./task-quick-start-restricted-mode.html["제한 모드에서 NetApp Console 시작하기 위한 단계를 따르세요."].
