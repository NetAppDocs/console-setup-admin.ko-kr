...
이러한 제한 사항은 NetApp Console 및 관리 설정(에이전트, 서비스형 소프트웨어(SaaS) 플랫폼 등)에만 적용됩니다.
...
=== SSL 복호화는 Cloud Volumes ONTAP에서만 지원됩니다

투명 프록시 서버는 Cloud Volumes ONTAP에만 연결된 에이전트에 대해 지원됩니다. 다른 NetApp 데이터 서비스와 함께 사용되는 에이전트에는 투명 프록시를 사용할 수 없습니다. NetApp 데이터 서비스를 Cloud Volumes ONTAP와 함께 사용하는 경우 투명 프록시 서버를 사용할 수 있는 Cloud Volumes ONTAP 전용 에이전트를 생성하십시오.
...
타사 에이전트나 VM 확장은 에이전트 VM에서 지원되지 않습니다.
