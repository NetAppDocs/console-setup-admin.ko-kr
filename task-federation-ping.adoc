...
PingFederate와의 페더레이션 연결을 설정하여 콘솔에 대한 SSO(Single Sign-On)를 활성화할 수 있습니다. 이 프로세스에는 콘솔을 서비스 공급자로 신뢰하도록 PingFederate 서버를 구성한 다음 콘솔에서 연결을 생성하는 작업이 포함됩니다.
...
*** *대상/엔티티 ID*의 경우  `urn:auth0:netapp-cloud-account:<fed-domain-name-pingfederate>`를 사용합니다. 여기서 <fed-domain-name-pingfederate>는 페더레이션의 도메인 이름입니다. 예를 들어 도메인이  `example.com`인 경우 대상/엔티티 ID는  `urn:auth0:netapp-cloud-account:fed-example-com-pingfederate`가 됩니다.
