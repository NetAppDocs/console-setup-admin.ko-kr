...
*** link:task-install-connector-aws-bluexp.html#aws-permissions-agent["에이전트 배포 정책 보기"]
...
.시작하기 전에
구독을 구성하려면 먼저 콘솔 에이전트를 만들어야 합니다. link:concept-connectors.html#connector-installation["콘솔 에이전트를 만드는 방법을 알아보세요"] .
