...
[role="lead"]
NetApp Console에서 AWS 계정의 클라우드 리소스를 배포하고 관리할 수 있도록 AWS 자격 증명을 추가하고 관리하세요. 여러 AWS Marketplace 구독을 관리하는 경우 자격 증명 페이지에서 각 구독에 서로 다른 AWS 자격 증명을 할당할 수 있습니다.
...
link:concept-accounts-aws.html["NetApp Console AWS 자격 증명 및 권한을 사용하는 방법을 알아보세요."].
...
NOTE: 콘솔에서 콘솔 에이전트를 배포한 경우 콘솔 에이전트를 배포한 계정에 대한 AWS 자격 증명이 자동으로 추가되었습니다. 이를 통해 리소스 관리에 필요한 권한이 있는지 확인할 수 있습니다.
...
IAM 사용자의 AWS 키를 Console에 제공하려면 해당 사용자에게 필요한 권한을 부여해야 합니다. Console IAM 정책은 Console에서 사용할 수 있는 AWS 작업 및 리소스를 정의합니다.
...
NOTE: 클라우드 제공업체의 새로운 자격 증명이 사용 가능해지는 데 몇 분이 걸릴 수 있습니다.
...
NetApp Console SaaS(Software as a Service) 애플리케이션이 역할을 수임할 수 있도록 IAM 역할을 설정합니다.
...
----
+
IAM에서 계정 간 리소스 액세스에 대한 자세한 내용은 link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["AWS Identity and Access Management(IAM) 문서"^]을(를) 참조하십시오.
...
.결과
이제 IAM 역할에 필요한 권한이 부여되었습니다. <<add-the-credentials,이제 콘솔에 추가할 수 있습니다.>>.
...
.시작하기 전에
방금 IAM 역할을 생성했다면 사용 가능해지기까지 몇 분 정도 걸릴 수 있습니다. NetApp Console에 자격 증명을 추가하기 전에 몇 분 정도 기다려 주세요.
...
. *조직 자격 증명* 페이지에서 *자격 증명 추가*를 선택하고 마법사의 단계를 따릅니다.
+
.. *자격 증명 위치*: *Amazon Web Services > 콘솔*을 선택합니다.
...
자세한 내용은 https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Amazon FSx for ONTAP용 콘솔 설명서"^]를 참조하십시오.
...
.시작하기 전에
구독을 구성하기 전에 Console 에이전트를 생성해야 합니다. link:concept-agent-install-options.html["콘솔 에이전트를 만드는 방법을 알아보세요"].
...
. 자격 증명을 기존 구독과 연결하려면 드롭다운 목록에서 구독을 선택하고 *Configure*를 선택합니다.
...
AWS Marketplace에서 구독을 신청할 때 마지막 단계는 구독을 조직과 연결하는 것입니다. 이 단계를 완료하지 않으면 조직에서 해당 구독을 사용할 수 없습니다.
...
.단계
. *관리 > 자격 증명*을 선택합니다.
. *조직 자격 증명* 페이지에서 자격 증명 세트에 대한 작업 메뉴를 선택한 다음 *자격 증명 편집*을 선택합니다.
...
TIP: 콘솔 에이전트와 연결된 인스턴스 프로필의 자격 증명은 삭제할 수 없습니다.
