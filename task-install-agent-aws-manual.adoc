...
---
= NetApp Console을 위해 AWS에서 Console 에이전트를 수동으로 배포합니다.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
AWS에서 실행되는 Linux 호스트에 Console 에이전트를 수동으로 배포할 수 있습니다. 자체 Linux 호스트에 Console 에이전트를 수동으로 배포하려면 호스트 요구 사항을 검토하고, 네트워크를 설정하고, AWS 권한을 준비하고, Console 에이전트를 배포한 다음, 준비한 권한을 부여해야 합니다.

AWS에 에이전트를 배포하면 사용자가 선택한 VPC에서 Linux를 실행하는 EC2 인스턴스와 Console 에이전트 소프트웨어가 시작됩니다.

.시작하기 전에
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["AWS에서 수동으로 배포해야 하는 경우 알아보기"].
...
+
사용하려는 NetApp 데이터 서비스에 따라 두 번째 정책을 생성해야 할 수도 있습니다. 표준 리전의 경우 권한은 두 개의 정책에 분산됩니다. AWS 관리형 정책의 최대 문자 크기 제한으로 인해 두 개의 정책이 필요합니다. link:reference-permissions-aws.html["콘솔 에이전트에 대한 IAM 정책에 대해 자세히 알아보세요."].
...
Console 에이전트를 생성한 AWS 계정에 Amazon S3 버킷이 있는 경우 *Systems* 페이지에 Amazon S3 스토리지 시스템이 자동으로 표시됩니다. https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["NetApp Console 에서 S3 버킷을 관리하는 방법을 알아보세요."^]
