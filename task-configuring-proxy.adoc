...
---
= NetApp Console 에이전트가 프록시 서버를 사용하도록 구성합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
IMPORTANT: 프록시를 구성한 후에는 프록시 유형을 변경할 수 없습니다. 프록시 유형을 변경해야 하는 경우 Console 에이전트를 제거하고 새 프록시 유형으로 새 에이전트를 추가해야 합니다.
...
콘솔 에이전트의 기존 투명 프록시 서버는 다음 방법을 사용하여 업데이트할 수 있습니다. `proxy update` 명령어를 사용하거나 투명 프록시 서버를 제거하려면 다음 방법을 사용하십시오. `proxy remove` 명령.  더 자세한 내용은 관련 문서를 참조하십시오.link:reference-agent-maint-console-proxy.html["에이전트 유지 관리 콘솔"] .
...
콘솔 에이전트가 프록시 서버를 사용하도록 구성한 경우 콘솔 에이전트에서 직접 API 트래픽을 활성화하여 프록시를 거치지 않고 클라우드 공급자 서비스로 직접 API 호출을 보낼 수 있습니다. AWS, Azure 또는 Google Cloud에서 실행되는 에이전트는 이 옵션을 지원합니다.
