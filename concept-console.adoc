...
summary: NetApp Console 모든 기능을 이용할 수 있는 SaaS 환경 또는 연결 제약이 있는 조직을 위한 자체 호스팅 옵션을 제공합니다. 
---
...
[role="lead"]
NetApp Console 모든 기능을 이용할 수 있는 SaaS 환경 또는 연결 제약이 있는 조직을 위한 자체 호스팅 옵션을 제공합니다.

NetApp Console 완전 관리형 SaaS 환경으로 사용하거나 자체 클라우드 환경에서 호스팅하는 솔루션으로 사용하는 두 가지 방식으로 사용할 수 있습니다. SaaS 옵션을 선택하면 console.netapp.com을 통해 콘솔에 액세스할 수 있으며, NetApp 모든 인프라, 인증 및 업데이트를 관리합니다. 웹 브라우저에서 모든 기능을 바로 이용할 수 있습니다. 자체 호스팅 옵션을 선택하면 AWS, Azure 또는 Google Cloud 환경에 콘솔 에이전트를 설치해야 합니다. 인터페이스는 로컬에서 실행되며, 에이전트는 인증 및 업데이트와 같은 필수 작업을 위해 NetApp 에 ​​대한 제한적인 외부 연결을 유지합니다.

조직의 인터넷 연결 상태가 표준이라면 SaaS 환경을 선택하세요. 가장 간편한 설정, 자동 업데이트, 그리고 모든 NetApp 스토리지 시스템, 데이터 서비스 및 고급 기능에 대한 완벽한 액세스를 누릴 수 있습니다. 콘솔에서 에이전트를 직접 추가할 수 있으며, NetApp 모든 유지 관리를 처리합니다.

정부 기관이나 규제 대상 산업과 같이 조직에서 엄격한 연결성, 보안 또는 규정 준수 제어가 필요한 경우 자체 호스팅 옵션을 선택하십시오. 더 많은 제어 권한과 네트워크 격리 기능을 확보할 수 있지만, 관리할 수 있는 NetApp 스토리지 시스템은 제한적이며 일부 기능은 사용할 수 없습니다. 콘솔 에이전트를 수동으로 설치하고 로컬 에이전트 인터페이스에서 콘솔에 직접 액세스할 수 있습니다. 클라우드 마켓플레이스를 통해 또는 수동으로 에이전트를 설치하면 에이전트는 NetApp 과만 외부 통신을 하며, 내부 인터넷 연결은 필요하지 않습니다.
...
image:diagram-standard-mode.png["공용 인터넷을 사용하는 표준 모드 아키텍처를 보여주는 개념 이미지로, SaaS 계층과 Auth0 인증 기능을 사용할 수 있습니다."]
...
image:diagram-restricted-mode.png["제한 모드의 개념적 이미지."]
