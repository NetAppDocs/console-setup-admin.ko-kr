...
---
= NetApp Console에서 Azure에 Console 에이전트 배포
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Console에서 Azure에 Console 에이전트를 배포하려면 네트워킹을 설정하고 Azure 권한을 준비한 다음 Console 에이전트를 배포해야 합니다.
...
. *가상 머신 인증* 페이지에서 Azure 권한 설정 방식과 일치하는 인증 옵션을 선택합니다.
+
...
. *세부 정보* 페이지에서 에이전트 이름을 입력하고 태그를 지정한 다음 NetApp Console에서 필요한 권한이 있는 새 역할을 생성할지, 아니면 link:reference-permissions-azure.html["필요한 권한"]을(를) 사용하여 설정한 기존 역할을 선택할지 선택합니다.
+
...
콘솔 에이전트를 만든 동일한 Azure 계정에 Azure Blob Storage가 있는 경우 Azure Blob Storage가 *시스템* 페이지에 자동으로 표시됩니다. https://docs.netapp.com/us-en/bluexp-blob-storage/index.html["NetApp Console 에서 Azure Blob 스토리지를 관리하는 방법을 알아보세요."^]
