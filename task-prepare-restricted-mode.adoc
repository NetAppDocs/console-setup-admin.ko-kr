...
Console 에이전트가 AWS Government 지역에 있는 경우: \https://*.blob.core.windows.net \https://cloudmanagerinfraprod.azurecr.io
 a| 
Console 에이전트가 AWS Government 지역에 설치된 경우 Console 에이전트 업그레이드용 이미지를 얻으려면.
...
Console 에이전트가 Azure Government 지역에 있는 경우: \https://*.blob.core.windows.net \https://occmclientinfragov.azurecr.us
 a| 
Console 에이전트가 Azure Government 지역에 설치된 경우 Console 에이전트 업그레이드용 이미지를 얻으려면.



 a| 
Console 에이전트가 상용 지역 또는 주권 지역에 있는 경우 다음 두 가지 엔드포인트 세트 중에서 선택할 수 있습니다.
