...
---
= NetApp Console에서 Google Cloud 프로젝트 및 권한에 대해 알아보세요.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. 콘솔에서 콘솔 에이전트를 시작할 수 있는 권한이 있는 Google 계정을 사용하여 콘솔 에이전트를 배포해야 합니다.
. 콘솔 에이전트를 배포할 때 에이전트에 사용할 https://cloud.google.com/iam/docs/service-accounts["서비스 계정"^]을(를) 선택하라는 메시지가 표시됩니다. 콘솔은 서비스 계정으로부터 Cloud Volumes ONTAP 시스템을 생성 및 관리하고, NetApp 백업 및 복구를 사용하여 백업을 관리하는 등의 권한을 얻습니다. 이러한 권한은 서비스 계정에 사용자 지정 역할을 연결하여 부여됩니다.
