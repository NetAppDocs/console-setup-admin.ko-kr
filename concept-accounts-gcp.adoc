...
. 콘솔 에이전트를 배포할 때 다음을 선택하라는 메시지가 표시됩니다. https://cloud.google.com/iam/docs/service-accounts["서비스 계정"^] 에이전트의 경우 콘솔은 서비스 계정으로부터 Cloud Volumes ONTAP 시스템을 생성하고 관리하고, NetApp 백업 및 복구를 사용하여 백업을 관리하는 등의 권한을 얻습니다. 서비스 계정에 사용자 정의 역할을 연결하여 권한을 제공합니다.
