...
---
= AWS 마켓플레이스에서 NetApp Console 에이전트 배포
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
AWS Marketplace에서 직접 AWS에 Console 에이전트를 배포합니다. AWS Marketplace에서 Console 에이전트를 배포하려면 네트워킹을 설정하고, AWS 권한을 준비하고, 인스턴스 요구 사항을 검토한 다음 Console 에이전트를 배포해야 합니다.

AWS에 에이전트를 배포하면 사용자가 선택한 VPC에서 Linux를 실행하는 EC2 인스턴스와 Console 에이전트 소프트웨어가 시작됩니다.

.시작하기 전에
* link:concept-agent-install-options.html#agent-marketplace-deploy["AWS Marketplace에서 배포해야 하는 시점을 알아보세요"].
