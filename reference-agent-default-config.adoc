...
summary: AWS, Azure, Google Cloud에서 인터넷 접속이 가능한 표준 배포에 대한 콘솔 에이전트 기본 구성과 온프레미스 환경에서 인터넷 접속이 불가능한 제한된 배포에 대한 콘솔 에이전트 기본 구성에 대해 알아보세요. 
---
= NetApp Console의 콘솔 에이전트 기본 구성
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Console 에이전트 서비스의 이름은 `occm`입니다.
*  `occm` 서비스는 MySQL 서비스에 의존합니다.
+
