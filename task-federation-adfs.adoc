...
---
= NetApp Console을 Active Directory Federation Services와 연동
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
AD FS 서버와의 페더레이션을 설정하여 NetApp Console에서 단일 로그인(SSO)을 활성화할 수 있습니다. 이 과정은 AD FS에서 Console을 서비스 공급자로 신뢰하도록 구성한 다음 NetApp Console에서 연결을 생성하는 것으로 시작됩니다.
...
+
.. 다음 스크립트를 실행하여 PowerShell을 사용하여 트러스트를 생성합니다.
+
[source, powershell]
----
...
+
*** 신뢰 당사자 식별자를 생성할 때 *YOUR_TENANT* 값을 사용하십시오. `netapp-cloud-account`
*** *WS-Federation 지원 활성화*를 선택할 때는 *YOUR_AUTH0_DOMAIN* 값을 사용하십시오: `netapp-cloud-account.auth0.com`
