...
summary: NetApp Console 관리의 새로운 기능에 대해 알아보십시오. 
---
= NetApp Console의 새로운 기능
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.Red Hat Enterprise Linux(RHEL) 9.6 및 Podman 5.4.0 지원
...
NetApp Console 알림을 보내는 이메일 주소가 *service@console.bluexp.netapp.com*에서 *service@console.netapp.com*으로 변경되었습니다. NetApp NetApp Console 이메일 알림을 계속 수신할 수 있도록 *service@console.netapp.com*을 허용하도록 이메일 규칙을 업데이트하는 것을 권장합니다.
...
BlueXP를 제한된 모드 또는 프라이빗 모드에서 사용하는 경우, 사용자 인터페이스는 클라우드 지역 또는 온프레미스에 배포된 Connector 가상 머신에서 액세스할 수 있습니다. 기본적으로 BlueXP는 자체 서명된 SSL 인증서를 사용하여 로컬 웹 브라우저에서 안전한 HTTPS 액세스를 제공합니다.
...
더 이상 필요하지 않은 권한을 제거하기 위해 Connector에 대한 AWS 및 Azure 정책을 업데이트했습니다. 해당 권한은 BlueXP 엣지 캐싱과 Azure 및 AWS의 Kubernetes 클러스터 검색 및 관리와 관련된 것이었습니다.
...
----
이러한 권한은 Cloud Volumes ONTAP에서 가상 머신 확장 집합을 지원하는 데 필요합니다. 기존 커넥터를 사용하고 이 새로운 기능을 사용하려면 Azure 자격 증명과 연결된 사용자 지정 정책에 이러한 권한을 추가해야 합니다.
...
RHEL 7 또는 CentOS 7에서 실행 중인 기존 에이전트가 있는 경우 NetApp은 해당 운영 체제를 RHEL 8 또는 9로 업그레이드하거나 변환하는 것을 지원하지 않습니다. 지원되는 운영 체제에 새 에이전트를 생성해야 합니다. 전환을 진행하려면 다음의 주요 단계를 따르십시오.
...
** https://docs.netapp.com/us-en/storage-management-fsx-ontap/use/task-create-fsx-system.html["ONTAP 시스템용 FSx 생성 또는 검색"^]
