...
.Cloud Volumes ONTAP 에 대한 추가 Google Cloud 권한
Google Cloud에서 Cloud Volumes ONTAP 관리를 지원하기 위해 콘솔 에이전트에 추가 권한이 필요합니다. 콘솔 에이전트에서 사용하는 서비스 계정에 다음 권한이 추가되었습니다. compute.forwardingRules.update

link:https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#gcp-permissions-change-log["권한 변경 로그를 확인하세요."]
