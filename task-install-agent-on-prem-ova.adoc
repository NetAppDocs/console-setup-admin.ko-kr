...
summary: VMware 사용자라면 OVA를 사용하여 vCenter에 Console 에이전트를 설치할 수 있습니다. OVA 다운로드 또는 URL은 NetApp Console을 통해 사용할 수 있습니다. 
---
= vCenter를 사용하여 온프레미스에 NetApp Console 에이전트를 설치합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
VMware 사용자라면 OVA를 사용하여 vCenter에 Console 에이전트를 배포할 수 있습니다. OVA 다운로드 또는 URL은 NetApp Console을 통해 이용할 수 있습니다.


NOTE: vCenter 도구와 함께 Console 에이전트를 배포하면 VM 웹 콘솔을 사용하여 유지 관리 작업을 수행할 수 있습니다. link:task-agent-vm-config.html["에이전트의 VM 콘솔에 대해 자세히 알아보세요."]
...
NOTE: 온프레미스에 Console 에이전트를 설치한 상태에서는 Google Cloud의 리소스를 관리할 수 없습니다. Google Cloud 리소스를 관리하려면 Google Cloud에 에이전트를 설치하세요.
...
.결과
이제 필요한 권한이 있는 IAM 사용자 액세스 키를 확보하셨을 것입니다. Console 에이전트를 설치한 후 Console에서 이러한 자격 증명을 Console 에이전트와 연결하십시오.
...
== vCenter 환경에 Console 에이전트를 설치하세요

NetApp은 vCenter 환경에 Console 에이전트를 설치하는 것을 지원합니다. OVA 파일에는 VMware 환경에 배포할 수 있는 사전 구성된 VM 이미지가 포함되어 있습니다. 파일 다운로드 또는 URL 배포는 NetApp Console에서 직접 이용할 수 있습니다. 이 파일에는 Console 에이전트 소프트웨어와 자체 서명된 인증서가 포함되어 있습니다.
...
. *OVA 포함*을 선택하세요.
. OVA를 다운로드하거나 vCenter에서 사용할 URL을 복사하도록 선택합니다.




[[deploy-the-agent-in-your-vcenter-environment]]
=== vCenter 환경에 에이전트를 배포하십시오

vCenter 환경에 로그인하여 에이전트를 배포하십시오.

.단계
. 환경에 필요한 경우 자체 서명된 인증서를 신뢰할 수 있는 인증서에 업로드하십시오. 설치 후 이 인증서를 교체합니다.
...
+
**** NetApp에서는 에이전트 설치 시 구성 검사를 포함하도록 이 확인란을 선택 해제하는 것이 좋습니다. 구성 검사는 에이전트가 필요한 엔드포인트에 대한 네트워크 액세스 권한이 있는지 확인합니다. 연결 문제로 인해 배포가 실패하는 경우 에이전트 호스트에서 유효성 검사 보고서 및 로그에 액세스할 수 있습니다. 경우에 따라 에이전트에 네트워크 액세스 권한이 있다고 확신하는 경우 검사를 건너뛸 수 있습니다. 예를 들어, 에이전트 업그레이드에 사용되는 link:reference-networking-saas-console-previous.html["이전 종료점"]을 계속 사용하는 경우 유효성 검사가 오류와 함께 실패합니다. 이를 방지하려면 유효성 검사 없이 설치하도록 확인란을 선택하십시오. link:reference-networking-saas-console-previous.html#update-endpoint-list["엔드포인트 목록을 업데이트하는 방법을 알아보세요"].
...
. *다음*을 선택하세요.
. *Ready to complete* 창에서 세부 정보를 검토한 다음 *Finish*를 선택하십시오.
+
