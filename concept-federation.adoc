...
---
= NetApp Console의 ID 페더레이션에 대해 알아보십시오.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: NetApp Console이 프라이빗 모드일 때는 페더레이션을 사용할 수 없습니다. link:concept-modes.html["NetApp Console 모드에 대해 자세히 알아보세요"^].
...
.프로토콜
* link:task-federation-adfs.html["Active Directory 페더레이션 서비스(AD FS)"]
* link:task-federation-saml.html["SAML(Security Assertion Markup Language) ID 공급자"]


.ID 공급자
* link:task-federation-entra-id.html["마이크로소프트 엔트라 ID"]
* link:task-federation-ping.html["핑페더레이트"]
...
[role="quick-margin-para"]
NetApp을 신뢰하도록 ID 공급자를 구성하려면 새 애플리케이션을 생성하고 ACS URL, 엔티티 ID 또는 기타 자격 증명 정보와 같은 세부 정보를 제공하십시오. 서비스 공급자 정보는 ID 공급자마다 다르므로 자세한 내용은 해당 ID 공급자의 설명서를 참조하십시오. 이 단계를 완료하려면 IdP 관리자와 협력해야 합니다.
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-4.png["네"] NetApp Console에서 페더레이션을 테스트하세요
[role="quick-margin-para"]
페더레이션 연결을 활성화하기 전에 테스트하세요. 콘솔의 페더레이션 페이지에서 테스트 옵션을 사용하여 테스트 사용자가 성공적으로 인증할 수 있는지 확인하세요. 테스트가 성공하면 연결을 활성화할 수 있습니다.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-5.png["다섯"] Console에서 연결을 활성화하세요
[role="quick-margin-para"]
