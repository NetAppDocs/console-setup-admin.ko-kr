...
.콘솔 에이전트를 사용하여
기본적으로 사용자는 ONTAP System Manager에 액세스할 때 ONTAP 자격 증명을 입력하라는 메시지가 표시되지 않습니다. 그러나 Console 관리자(Organization admin 역할 보유)는 사용자가 ONTAP 자격 증명을 입력하라는 메시지가 표시되도록 Console을 구성할 수 있습니다. 이 설정이 활성화되면 사용자는 매번 ONTAP 자격 증명을 입력해야 합니다.
