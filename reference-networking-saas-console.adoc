...
* 온프레미스 또는 클라우드에 설치하는 콘솔 에이전트입니다. 콘솔 에이전트.



NOTE: NetApp 4.0.0을 통해 콘솔 및 콘솔 에이전트에 필요한 네트워크 엔드포인트를 줄여 보안을 강화하고 배포를 간소화했습니다. 중요한 점은 버전 4.0.0 이전의 모든 배포가 계속해서 완벽하게 지원된다는 것입니다.  기존 에이전트에서는 이전 엔드포인트를 계속 사용할 수 있지만 NetApp 에이전트 업그레이드가 성공적으로 완료되었음을 확인한 후 현재 엔드포인트에 대한 방화벽 규칙을 업데이트할 것을 강력히 권장합니다.link:reference-networking-saas-console-previous.html["엔드포인트 목록을 업데이트하는 방법을 알아보세요."]



== NetApp Console 및 콘솔 에이전트가 연락한 엔드포인트

배포하는 각 에이전트와 NetApp Console 액세스하는 각 컴퓨터는 아래 나열된 엔드포인트에 연결되어 있어야 합니다.

클라우드 공급자에 배포된 콘솔 에이전트는 해당 클라우드 공급자와 관련된 엔드포인트에 액세스해야 합니다.
...
| Console 에이전트가 AWS Government 지역에 있는 경우: \https://*.blob.core.windows.net \https://cloudmanagerinfraprod.azurecr.io | Console 에이전트가 AWS Government 지역에 설치된 경우 Console 에이전트 업그레이드용 이미지를 얻으려면. 


| Console 에이전트가 Azure Government 지역에 있는 경우: \https://*.blob.core.windows.net \https://occmclientinfragov.azurecr.us | Console 에이전트가 Azure Government 지역에 설치된 경우 Console 에이전트 업그레이드용 이미지를 얻으려면. 
...
| Console 에이전트가 상업 지역 또는 주권 지역에 설치된 경우 Console 에이전트 업그레이드를 위한 이미지를 얻으려면 다음을 수행하십시오. [cols=3*,options="header,autowidth"] 
|===
...
== 클라우드 공급자 엔드포인트가 콘솔 에이전트에 연결되었습니다.

콘솔 에이전트가 클라우드 공급자에 배포된 경우 추가 엔드포인트에 액세스할 수 있어야 합니다.

콘솔 에이전트를 설치하기 전에 클라우드 공급자 네트워크 엔드포인트 액세스를 설정하세요.
...
콘솔 에이전트는 NetApp 워크로드에 대해 다음 엔드포인트에 액세스할 수 있어야 합니다.
