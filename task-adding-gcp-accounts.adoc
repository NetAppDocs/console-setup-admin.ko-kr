...
---
= 콘솔 에이전트 자격 증명을 Google Cloud 구독과 연결하세요.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Google Cloud에서 콘솔 에이전트를 만들면 연관된 자격 증명을 사용하여 Cloud Volumes ONTAP 및 NetApp 데이터 서비스를 포함하는 NetApp Intelligent Services 를 구독할 수 있습니다.  서비스 비용을 지불하는 데 사용할 자격 증명을 가진 에이전트와 구독을 연결해야 합니다.
...
현재 마켓플레이스 구독을 새 구독으로 교체하면 기존 및 새로운 Cloud Volumes ONTAP 시스템의 구독이 업데이트됩니다.
...
Google Cloud Marketplace를 통해 NetApp 데이터 서비스를 구독하는 경우 권한이 잘못되었거나 콘솔로의 리디렉션이 누락되어 구독이 실패할 수 있습니다. 이런 일이 발생하면 다음 단계에 따라 구독 절차를 완료하세요.
