...
---
= Google Cloud에 NetApp Console 에이전트를 수동으로 배포합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Linux 호스트에 Console 에이전트를 수동으로 배포하려면 호스트 요구 사항을 검토하고, 네트워크를 설정하고, Google Cloud 권한을 준비하고, Google Cloud API를 사용 설정하고, Console을 설치한 다음, 준비한 권한을 부여해야 합니다.

Google Cloud에 에이전트를 배포하면 사용자가 선택한 VPC에서 Linux를 실행하는 VM 인스턴스와 Console 에이전트 소프트웨어가 시작됩니다.

.시작하기 전에
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["Google Cloud에서 수동으로 배포해야 하는 경우 알아보기"].
