...
image:diagram_azure_authentication.png["콘솔이 API 호출을 하기 전에 Microsoft Entra ID로부터 인증 및 권한 부여를 받는 과정을 보여주는 개념 이미지입니다. 콘솔 역할은 Active Directory에서 권한을 정의합니다. 이는 하나 이상의 Azure 구독 및 콘솔 애플리케이션을 나타내는 서비스 주체 개체와 연결됩니다."]
