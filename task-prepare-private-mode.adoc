...
---
= NetApp Console 프라이빗 모드에서 배포 준비
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* python3 설치
* podman-compose 패키지 설치(Red Hat 버전에 따라 다름)
...
* HTTPS 인증서
+
개인 모드에서는 NetApp Console이 외부 트래픽을 전송하는 유일한 경우는 클라우드 공급자에게 Cloud Volumes ONTAP 시스템을 생성하기 위한 트래픽입니다.
...
콘솔 에이전트를 클라우드에 설치하고 Cloud Volumes ONTAP 시스템을 생성하려는 경우 NetApp Console에는 클라우드 공급자 권한이 필요합니다. 클라우드 공급자에서 권한을 설정한 다음 콘솔 에이전트 인스턴스를 설치한 후 해당 권한을 콘솔 에이전트 인스턴스와 연결해야 합니다.
...
Azure 포털, Azure PowerShell, Azure CLI 또는 REST API를 사용하여 Azure 사용자 지정 역할을 만들 수 있습니다. 다음 단계는 Azure CLI를 사용하여 역할을 만드는 방법을 보여줍니다. 다른 방법을 사용하려면 https://learn.microsoft.com/en-us/azure/role-based-access-control/custom-roles#steps-to-create-a-custom-role["Azure 설명서"^]을 참조하십시오.
...
+
image:screenshot_azure_app_ids.gif["Microsoft Entra ID에서 애플리케이션의 애플리케이션(클라이언트) ID와 디렉터리(테넌트) ID를 보여주는 스크린샷입니다."]
...
.결과
이제 서비스 주체가 설정되었으며 애플리케이션(클라이언트) ID, 디렉터리(테넌트) ID 및 클라이언트 암호 값을 복사했을 것입니다. Azure 자격 증명을 추가할 때 NetApp Console에 이 정보를 입력하십시오.
