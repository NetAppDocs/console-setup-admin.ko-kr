...
summary: NetApp Console에서 AWS에 Console 에이전트를 직접 배포할 수 있습니다. Console에서 AWS에 Console 에이전트를 배포하기 전에 네트워킹을 설정하고 AWS 권한을 준비해야 합니다. 
---
= NetApp Console에서 AWS에 Console 에이전트 배포
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Console에서 AWS에 Console 에이전트를 직접 배포할 수 있습니다. Console에서 AWS에 Console 에이전트를 배포하기 전에 네트워킹을 설정하고 AWS 권한을 준비해야 합니다.
...
. 다음 정책을 복사하여 붙여넣으세요.
+
이 정책에는 AWS 콘솔에서 콘솔 에이전트를 시작하는 데 필요한 권한만 포함되어 있습니다.  콘솔이 콘솔 에이전트를 생성하면 콘솔 에이전트가 AWS 리소스를 관리할 수 있도록 하는 새로운 권한 집합이 콘솔 에이전트에 적용됩니다. link:reference-permissions-aws.html["콘솔 에이전트 자체에 필요한 권한 보기"].
...
Console 웹 기반 콘솔에서 Console 에이전트를 직접 생성하세요.
...
* 콘솔에서 콘솔 에이전트를 생성하면 에이전트에 대한 IAM 역할과 프로필이 생성됩니다. 이 역할에는 콘솔 에이전트가 AWS 리소스를 관리할 수 있는 권한이 포함됩니다. 향후 릴리스에서 새로운 권한이 추가되면 역할이 업데이트되도록 하세요. link:reference-permissions-aws.html["콘솔 에이전트에 대한 IAM 정책에 대해 자세히 알아보세요."].
...
. *소개* 페이지에서 프로세스 개요를 검토하십시오.
. *AWS 자격 증명* 페이지에서 AWS 리전을 지정한 다음 인증 방법을 선택합니다. 인증 방법은 NetApp Console에서 수임할 수 있는 IAM 역할이거나 AWS 액세스 키와 비밀 키입니다.
+
...
. *네트워크* 페이지에서 에이전트에 사용할 VPC, 서브넷 및 키 페어를 지정하고 공용 IP 주소를 활성화할지 여부를 선택한 다음 선택적으로 프록시 구성을 지정합니다.
+
